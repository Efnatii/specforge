export class FindPanel {
  ...
  render(state = {}) {
    ...
    scope.innerHTML = '<option value="sheet">Active sheet</option><option value="workbook">Workbook</option>';
    scope.value = state.scope || "sheet";

    const caseBox = this.makeCheck("Case", Boolean(state.matchCase));
    const wholeBox = this.makeCheck("Whole", Boolean(state.wholeCell));
    const regexBox = this.makeCheck("Regex", Boolean(state.useRegex));
    ...
    const searchBtn = this.btn("Find", () => this.callbacks.onSearch?.(this.getQuery()));
    const prevBtn = this.btn("Prev", () => this.callbacks.onPrev?.());
    const nextBtn = this.btn("Next", () => this.callbacks.onNext?.());
    const oneBtn = this.btn("Replace", () => this.callbacks.onReplaceOne?.({ ...this.getQuery(), replacement: replace.value }));
    const allBtn = this.btn("Replace all", () => this.callbacks.onReplaceAll?.({ ...this.getQuery(), replacement: replace.value }));
