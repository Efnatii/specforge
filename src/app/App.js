...
export class App {
  ...
  async runFind(query) {
    ...
    try {
      const { promise } = this.jobQueue.enqueue({ type: "BUILD_FIND_INDEX", title: "Build find index", workerOp: "BUILD_FIND_INDEX", workerPayload: { scope: query.scope, normalizedWorkbook: this.currentWorkbook, edits: state.edits, calcSnapshot: state.calc, sheetName: activeSheetName } });
      ...
      this.toast.show(`Find index failed: ${error.message}`, "error");
    }
  }
  ...
  mountChaosPanel() {
    ...
    this.chaosPanel = new ChaosPanel(shell, this.chaosService);
